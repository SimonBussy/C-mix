...
import numpy as np
import pandas as pd
from QNEM.base import BaseClass
...
class Learner(BaseClass):
    ...
    def __init__(self, **kwargs):
        ...
        self.set_params(verbose=True, print_every=10)
    ...
    def _start_solve(self):
        # Reset history
        self.history.clear()
        self._set_info(start=self._get_now())
        self._start = time()
        if self.get_params("verbose"):
            print "Launching the solver " + self.get_info("class") + "..."

    def _end_solve(self):
        t = time()
        self._set_info(elapsed=t - self._start)
        if self.get_params("verbose"):
            print "Done solving using " + self.get_info("class") + " in " \
                        + "%.2e seconds" % self.get_info("elapsed")
...
class MixtureGeoms(Learner):
    ...
    def __init__(self, **kwargs):
        ...
        self.set_params(max_iter=100, verbose=True, init="random",
                        print_every=1, tol=1e-5)
        self.set_params(**kwargs)
        self._init_coeffs(4)
    ...
    def fit(self, Y, delta, model):
        ...
        with two components
        """
        init = self.get_params("init")
        verbose = self.get_params("verbose")
        max_iter = self.get_params("max_iter")
        print_every = self.get_params("print_every")
        tol = self.get_params("tol")
        self._start_solve()
        if init == "random":
            # Split at random the sample with proba 0.5
            n_samples = Y.shape[0]
            pi = 0.5
...
class qnem(Learner):
    ...
    """
    def __init__(self,l_elasticNet=0.,eta=.1,max_iter=100,verbose=True,
                 init="random",print_every=1,tol=1e-5,warmStart=False,
                 model="C-mix",intercept=False,**kwargs):
        Learner.__init__(self,l_elasticNet=l_elasticNet,eta=eta,
                         max_iter=max_iter,verbose=verbose,init=init,
                         print_every=print_every,tol=tol,
                         warmStart=warmStart,model=model)
        ...
        self.scores = None
        self.l_elasticNet_best = None
        self.l_elasticNet_chosen = None
        self.grid_elasticNet = None
    ...
    def _func_pen(self, coeffs_ext):
        ...
        """
        l_elasticNet = self.get_params("l_elasticNet")
        eta = self.get_params("eta")
        n_features = self.n_features
        
        coeffs = coeffs_ext[:n_features] - coeffs_ext[n_features:]
        
        return l_elasticNet * ( (1. - eta) * coeffs_ext.sum() \
                                + 0.5 * eta * np.linalg.norm(coeffs) ** 2)
        
    def _grad_pen(self, coeffs_ext):
        """
        ...
        grad += l_elasticNet * (1 - eta)
        # Gradient of ridge penalization
        grad_pos = l_elasticNet * eta
        grad[:n_features] += grad_pos
        grad[n_features:] -= grad_pos
        return grad 
    ...
    def _grad_sub_obj(self, X, q, coeffs_ext):
        ...
        grad_pen = self._grad_pen(coeffs_ext)
        u = coeffs_0 + X.dot(coeffs)
        ...
        if self.intercept:
            ...
            grad_pen = np.concatenate([[0], grad_pen[:n_features], [0], 
                                       grad_pen[n_features:]])
        grad = (X*(q-self.logistic_grad(u)).reshape(n_samples,1)).mean(axis=0)
    ...
    def fit(self, X, Y, delta):
        ...
        print_every = self.get_params("print_every")
        tol = self.get_params("tol")
        warmStart = self.get_params("warmStart")
        ...
        coeffs_ext[:n_features] = coeffs

        func_obj = self._func_obj
        func_sub_obj = self._func_sub_obj
        grad_sub_obj = self._grad_sub_obj
        
        # We initialize p0 and p1 by fitting a censoring mixture 
        # of geometrics
        mixt_geoms = MixtureGeoms(verbose=False, init=init)
        ...
        for n_iter in range(max_iter):
            ...
            coeffs_ext = fmin_l_bfgs_b(
                            func=lambda coeffs_ext_ : func_sub_obj(
                                                X, q, coeffs_ext_),
                            x0=x0,
                            fprime=lambda coeffs_ext_ : grad_sub_obj(
                                                X, q, coeffs_ext_),
                            disp=False, 
                            bounds=bounds, 
                            maxiter=50, 
    ...
    def score(self, X, Y, delta, metric):
        ...
        if metric == 'C-index':
            return self._c_index(Y, delta, self.predict_proba(X, 
                                                 self.intercept, self.coeffs))

    def cross_validate(self,X,Y,delta,n_folds=3,eta=0.1, adaptative_grid=True,
                       grid_size=50, grid_elasticNet=np.array([0]),
                       shuffle=True, verbose=True,metric='log_lik'):
        """
        Apply n_folds cross-validation using the given data, to select the best
        penalization parameter
        """
        from sklearn.cross_validation import KFold
        n_samples = Y.shape[0]
        cv = KFold(n_samples, n_folds=n_folds, shuffle=shuffle)
        self.grid_elasticNet = grid_elasticNet
        ...
        if adaptative_grid:
            # from KKT condition:
            gamma_max = 1./np.log(10.) * np.log(1./(1.-eta) * (.5/n_samples) \
                        * np.absolute(X).sum(axis=0).max()) 
            grid_elasticNet = np.logspace(gamma_max-4, gamma_max, grid_size)        
        
        learners = [
            qnem(verbose=False,tol=tol,eta=eta,warmStart=warmStart,
                 model=model,intercept=self.intercept) 
            for _ in range(n_folds)
                    ]
                    
        n_grid_elasticNet = grid_elasticNet.shape[0]
        scores = np.empty((n_grid_elasticNet, n_folds))
        if verbose is not None:
            verbose = self.get_params("verbose")
        for idx_elasticNet, l_elasticNet in enumerate(grid_elasticNet):
            if verbose:
                print "Testing l_elasticNet=%.2e" % l_elasticNet,"on fold",
            for n_fold, (idx_train, idx_test) in enumerate(cv):
                ...
                learner.set_params(l_elasticNet=l_elasticNet)
        ...
        l_elasticNet_best = grid_elasticNet[idx_best]
        ...
        l_elasticNet_chosen = grid_elasticNet[idx_chosen]
        self.grid_elasticNet = grid_elasticNet
        self.l_elasticNet_best = l_elasticNet_best
        self.l_elasticNet_chosen = l_elasticNet_chosen
